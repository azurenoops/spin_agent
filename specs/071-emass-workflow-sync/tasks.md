...
**Feature**: 071-emass-workflow-sync | **Issue**: #891
...
> **Implementation note**: The Feature 041 baseline already emitted OSCAL 1.1.2 when
> this work began. Phase 1 was verified through regression tests rather than a version
> rewrite. Round-trip sync uses dedicated non-destructive ClosedXML adapters because the
> existing onboarding import path mutates SPIN state and only recognizes Controls exports.
...
- [x] T001 [US1] Upgrade `BuildOscalPoam()` in `src/Ato.Copilot.Agents/Compliance/Services/EmassExportService.cs`:
...
- [x] T002 [US1] Upgrade `BuildOscalAssessmentResults()` in `src/Ato.Copilot.Agents/Compliance/Services/EmassExportService.cs`:
  - Set `metadata.oscal-version` to `"1.1.2"`
...
- [x] T003 [P] [US1] Add/update unit tests in `tests/Ato.Copilot.Tests.Unit/Compliance/EmassExportServiceTests.cs`:
...
- [x] T004 [P] Create `EmassConflict` entity in `src/Ato.Copilot.Core/Models/Compliance/EmassConflict.cs`:
...
- [x] T005 [P] Create `EmassWorkflowStatus` DTO in `src/Ato.Copilot.Core/Dtos/Dashboard/EmassWorkflowDtos.cs`:
...
- [x] T006 [P] Create service interfaces in `src/Ato.Copilot.Core/Interfaces/Compliance/`:
...
- [x] T007 Register new DbSet `EmassConflicts` in `AtoCopilotContext`, configure relationships and indexes in `OnModelCreating`, create idempotent SQL Server/SQLite schema addition
...
- [x] T008 [US2] [US5] Implement `EmassExportReadinessService` in `src/Ato.Copilot.Agents/Compliance/Services/EmassExportReadinessService.cs`:
...
- [x] T009 [P] [US5] Update `EmassExportService` Excel export methods to include `DitprId` and `EmassId` in the correct eMASS template columns; update OSCAL builders to populate `metadata.system-id` array with both identifier schemes

- [x] T010 [P] [US2] Add unit tests in `tests/Ato.Copilot.Tests.Unit/Compliance/EmassExportReadinessServiceTests.cs`:
...
- [x] T011 [US3] Implement `EmassRoundTripSyncService` in `src/Ato.Copilot.Agents/Compliance/Services/EmassRoundTripSyncService.cs`:
...
- [x] T012 [US3] Implement conflict resolution method in `EmassRoundTripSyncService`:
...
- [x] T013 [P] [US3] Add unit tests in `tests/Ato.Copilot.Tests.Unit/Compliance/EmassRoundTripSyncServiceTests.cs`:
...
- [x] T014 [US4] Implement `EmassWorkflowStatusService` in `src/Ato.Copilot.Agents/Compliance/Services/EmassWorkflowStatusService.cs`:
...
- [x] T015 [P] [US4] Add unit tests in `tests/Ato.Copilot.Tests.Unit/Compliance/EmassWorkflowStatusServiceTests.cs`:
...
- [x] T016 [P] [US4] Add `GET /api/systems/{id}/emass/status` to `src/Ato.Copilot.Mcp/Endpoints/EmassWorkflowEndpoints.cs`:
  - Returns `EmassWorkflowStatus` JSON
  - RBAC: ISSO, ISSM, AO

- [x] T017 [P] [US2] Add `GET /api/systems/{id}/emass/readiness` to `EmassWorkflowEndpoints.cs`:
  - Returns `EmassExportReadinessResult`
  - RBAC: ISSO, ISSM, AO

- [x] T018 [P] [US3] Add sync endpoints to `EmassWorkflowEndpoints.cs`:
...
- [x] T019 [P] Register `EmassWorkflowEndpoints` in `src/Ato.Copilot.Mcp/Program.cs` (or equivalent startup)
...
- [x] T020 [P] [US4] [US2] Implement `emass_get_workflow_status` and `emass_check_export_readiness` MCP tools in `src/Ato.Copilot.Agents/Compliance/Tools/EmassWorkflowTools.cs`:
...
- [x] T021 [P] [US4] [US2] Register new tools in the MCP tool registry; update `docs/architecture/agent-tool-catalog.md` with parameter tables, response schemas, RBAC, and example invocations
...
- [x] T022 [P] [US4] Create `EmassStatusPage` React component at `src/Ato.Copilot.Dashboard/src/pages/EmassStatus.tsx`:
...
- [x] T023 [P] [US3] Create `EmassConflictList` React component at `src/Ato.Copilot.Dashboard/src/components/EmassConflictList.tsx`:
...
- [x] T024 [P] Add `emass-status.ts` API client in `src/Ato.Copilot.Dashboard/src/api/emass-status.ts`:
...
- [x] T025 Register new route `/systems/:id/emass/status` in dashboard router
...
- [x] T026 [P] Structured Serilog logging for sync runs: log batch ID, conflicts created, elapsed time
- [x] T027 [P] Update `docs/guides/emass-workflow.md` (new guide) covering: status page, readiness check, round-trip sync workflow, conflict resolution
- [x] T028 [P] Update `docs/getting-started/isso.md` with eMASS workflow sync section
- [x] T029 Run `dotnet test` end-to-end; verify all Feature 041 tests still pass; verify NFR-001 sync performance on mock data (400 controls, 150 POA&M items < 30 s)
- [x] T030 Code review: CancellationToken propagation, RBAC enforcement, API envelope consistency, focused unit/integration/dashboard/E2E coverage
