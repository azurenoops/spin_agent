...
| [#1021](https://github.com/azurenoops/spin_agent/issues/1021) contributor/classification/coverage domain | Open; explicit contract not present in the audited branch | Many-to-many provider contributors, classification distinct from service category, explicit provider/customer duties | Hard gate for #1027/#1034; extend existing models once rather than creating UI-only contributor semantics |
| [#1023](https://github.com/azurenoops/spin_agent/issues/1023) provider baseline versus system ATO | Open; explicit contract not present in the audited branch | Provider source authorization metadata kept separate from system authorization decisions | Hard gate for #1026/#1033; missing provider authorization is displayed as not recorded |
