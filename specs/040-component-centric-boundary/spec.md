...
- **FR-028**: Authorization-boundary candidate lists MUST exclude Person components and include active tenant org-level Place, Thing, and Policy components that are not already assigned to the selected boundary.
- **FR-029**: Authorization-boundary candidate lists MUST include published global CSP inherited components as read-only references without copying them into tenant-local SystemComponent storage.
- **FR-030**: A BoundaryComponentAssignment MUST reference exactly one source: either a tenant-local SystemComponent or a global CspInheritedComponent. Existing SystemComponent assignments remain valid after schema migration.
- **FR-031**: Boundary assignment endpoints MUST reject Person components and CSP components that are not published, including direct API calls that bypass the dashboard.
- **FR-032**: Every boundary candidate and assignment operation MUST validate that the route system and boundary belong to the active tenant and that the boundary belongs to the route system.
- **FR-033**: Tenant org-level candidates MUST remain tenant-filtered, while published CSP candidates remain globally readable. Cross-tenant systems, boundaries, components, and assignments MUST return not found and MUST NOT create or mutate data.
- **FR-034**: Component Inventory, Boundary Management, and system-intake boundary pickers MUST use the same source-aware candidate contract and dispatch assignments using the candidate source.
- **FR-035**: Boundary assignment endpoints MUST reject null or unknown source labels instead of treating them as tenant-local component references.
...
- **BoundaryComponentAssignment** (new): Join entity linking exactly one tenant-local SystemComponent or global CspInheritedComponent reference to an AuthorizationBoundaryDefinition within a RegisteredSystem. Tracks source, per-boundary scope, exclusion rationale, and inheritance provider. Replaces the scope-tracking role of the AuthorizationBoundary entity.
