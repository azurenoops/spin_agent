...
## Dashboard Development (Feature 030)

### Frontend Stack

The Visual Compliance Dashboard is a standalone React SPA in `src/Ato.Copilot.Dashboard/`.
...
- **Types**: All TS types mirror backend DTOs in `src/types/dashboard.ts`
- **API Client**: Centralized Axios client with auth interceptor in `src/api/client.ts`
- **Polling**: Use the `usePolling` hook (pauses when tab is blurred) for auto-refresh
- **Components**: Organized by type: `cards/`, `charts/`, `forms/`, `layout/`
...
Dashboard backend services are in `src/Ato.Copilot.Mcp/Services/`:

| Service | Purpose |
|---------|---------|
| `DashboardService` | Portfolio, system detail, heatmap, trends queries |
| `CapabilityService` | Security capabilities CRUD, mappings, gap analysis |
| `ComponentService` | System component CRUD with capability linking |
| `NarrativeTemplateService` | Template-based narrative generation |
| `ComplianceTrendSnapshotService` | Daily background snapshot capture |

### Running Backend Tests

```bash
dotnet test tests/Ato.Copilot.Tests.Unit -- --filter "FullyQualifiedName~ComponentService|DashboardService|CapabilityService|NarrativeTemplate|TrendSnapshot"
```
