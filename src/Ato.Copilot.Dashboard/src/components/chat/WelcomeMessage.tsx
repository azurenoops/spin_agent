...
export default function WelcomeMessage({ context, onSendExample }: WelcomeMessageProps) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center p-6">
      <div className="mb-4 rounded-full bg-indigo-50 p-3 dark:bg-indigo-950">
      ...
      <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Security Posture Intelligence Navigator</h3>

      {/* Phase badge + guidance */}
      {phase ? (
        <div className="mt-3 w-full rounded-lg border border-gray-100 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-900">
          <div className="flex items-center gap-2">
            ...
            {context?.systemName && (
              <span className="truncate text-xs text-gray-500 dark:text-gray-400">{context.systemName}</span>
            )}
          </div>
          <p className="mt-2 text-xs leading-relaxed text-gray-500 dark:text-gray-400">{guidance}</p>
          {nextPhase && (
            <p className="mt-1 text-[10px] text-gray-400">
              Next phase: <span className="font-medium text-gray-600 dark:text-gray-300">{nextPhase}</span>
            </p>
          )}
        </div>
      ) : (
      ...
      <div className="mt-4 w-full space-y-2">
        ...
        {suggestions.map((s) => (
          <button
            ...
            className="flex w-full items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-left text-sm text-gray-600 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 transition-colors dark:border-gray-700 dark:text-gray-300 dark:hover:border-indigo-500 dark:hover:bg-indigo-950 dark:hover:text-indigo-200"
          >
