...
import { RMF_ROLE_OPTIONS, type ResolvedRoleAssignment, type RmfRole } from '../../types/roles';
...
function roleLabel(role: string): string {
  return RMF_ROLE_OPTIONS.find((r) => r.value === role)?.label ?? role;
}
...
export default function GateActionDialog({ action, systemId, onClose, onSuccess }: GateActionDialogProps) {
  ...
  // Roles state
  const [roleSelected, setRoleSelected] = useState<RmfRole>(RMF_ROLE_OPTIONS[0]!.value);
          ...
          {action === 'roles' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={roleSelected}
                  onChange={(e) => setRoleSelected(e.target.value as RmfRole)}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                >
                  {RMF_ROLE_OPTIONS.map((o) => (
