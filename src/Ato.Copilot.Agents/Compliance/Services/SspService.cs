...
public class SspService : ISspService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // ─── Fail-loud grounding guard (Fix #685) — runs before export ──────────
        // Reject export if any narrative used in §10 is still a TemplateScaffold
        // (fabricated default) or AiSuggested without an approved version.
        // This mirrors the fail-closed pattern from #790/ResponseCacheService.
        ...
        foreach (var ci in narratives)
        {
            ...
            else if (isUngroundedScaffold)
                groundingViolations.Add($"Control {ci.ControlId}: AI-suggested narrative has no approved version — reviewer gate required before export.");
        }

        if (groundingViolations.Count > 0)
        {
            _logger.LogError(
                "UNGROUNDED_CONTENT: SSP export blocked for system '{SystemId}' — {Count} grounding violation(s). " +
                "Resolve [SOURCE MISSING] markers and obtain reviewer approval before exporting.",
                systemId, groundingViolations.Count);
            // Add violations as warnings to the document so callers can surface them.
            // For a finalized/export scenario, callers should treat any Warnings containing
            // GROUNDING_VIOLATION as a hard block (status 422/UNGROUNDED_CONTENT).
            foreach (var v in groundingViolations)
                doc.Warnings.Add($"GROUNDING_VIOLATION: {v}");
        }
            ...
            foreach (var controlId in family.OrderBy(c => c))
            {
                ...
                if (narrativeMap.TryGetValue(controlId, out var impl))
                {
                    ...
                    else if (!string.IsNullOrWhiteSpace(impl.Narrative))
                    {
                        ...
                        // the grounding guard above will catch them and callers can block export.
                        sb.AppendLine($"**Approval Status**: {impl.ApprovalStatus} ⚠️ No approved version — reviewer gate required");
                        sb.AppendLine();
                        if (impl.AiSuggested || impl.IsAutoPopulated)
                        {
                            sb.AppendLine($"[SOURCE MISSING: This narrative was generated by template/AI and has not been approved by a human reviewer. " +
    ...
    /// <summary>
    /// Generates a non-fabricating scaffold for a customer-implemented control.
    /// Every technology/protocol claim that cannot be sourced from the system record
    /// is replaced with a [SOURCE MISSING: &lt;field&gt;] marker. The template NEVER
    /// asserts MFA, CAC/PIV, TLS 1.2+, or any other implementation fact unless a
    /// future caller passes sourced evidence — it only emits blank + marker.
    /// </summary>
    /// <remarks>Fix #685: removed all hardcoded technology assertions.</remarks>
    internal static string GenerateCustomerNarrativeTemplate(
        string family,
        string controlId,
        RegisteredSystem system)
    {
        // Hosting environment is sourced from the system record. If absent, emit marker.
        var hostingEnv = !string.IsNullOrWhiteSpace(system.HostingEnvironment)
            ? system.HostingEnvironment
            : "[SOURCE MISSING: HostingEnvironment]";

        // Per Banner investigation (#685): do NOT assert any specific technology
        // (MFA, CAC/PIV, TLS 1.2+, Defender, etc.) without a sourced evidence record.
        // All family templates below produce a minimal factual frame plus a prominent
        // [SOURCE MISSING] block that the reviewer must replace before export.
        return family switch
        {
            "AC" =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Describe the actual access-control mechanisms implemented (e.g., identity provider, " +
                $"conditional access policies, MFA enforcement, RBAC configuration). Do not assert specific identity providers, conditional access policies, or multi-factor requirements unless sourced.]",

            "AU" =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Describe the actual audit and logging infrastructure (e.g., logging platform, " +
                $"retention configuration, protection measures). Do not assert specific tools unless sourced.]",

            "CM" =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Describe the actual configuration management toolchain and baseline enforcement " +
                $"mechanisms. Do not assert IaC tools or policy engines unless sourced.]",

            "IA" =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Describe the actual identity and authentication mechanisms (e.g., identity provider, " +
                $"authenticator types, identity proofing standards). Do not assert specific authenticator hardware or multi-factor mechanisms unless sourced from system evidence.]",

            "SC" =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Describe the actual system and communications protection controls (e.g., network " +
                $"segmentation, firewall configuration, encryption protocols and versions). " +
                $"Do not assert specific encryption versions or network security products unless sourced from system evidence.]",

            "SI" =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Describe the actual system integrity and monitoring capabilities (e.g., vulnerability " +
                $"scanning tools, SIEM/alerting platform, integrity verification approach). Do not assert specific " +
                $"products unless sourced.]",

            _ =>
                $"The {system.Name} system implements {controlId} within the {hostingEnv} environment. " +
                $"[SOURCE MISSING: Implementation details for {controlId} must be documented by the system " +
                $"engineering team before this narrative can be used in an ATO package.]"
        };
    }
