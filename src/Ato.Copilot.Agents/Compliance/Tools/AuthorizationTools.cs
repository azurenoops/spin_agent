...
/// Creates a RiskAcceptance record linked to the system's active AuthorizationDecision.
/// Returns an error if no active authorization exists (guards the AO workflow gate).
/// RBAC: Compliance.AuthorizingOfficial ONLY
/// </summary>
public class AcceptRiskTool : BaseTool
{
    ...
    public override string Description =>
        "Accept risk on a specific finding and control. Creates a RiskAcceptance record " +
        "linked to the system's active authorization. Requires an active ATO/IATO. " +
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                status = "success",
                data = FormatAcceptance(acceptance),
                metadata = Meta(sw)
            }, JsonOpts);
        }
        catch (InvalidOperationException ex)
    ...
    private static object FormatAcceptance(RiskAcceptance r) => new
    {
        ...
        is_active = r.IsActive && r.ExpirationDate > DateTime.UtcNow,
