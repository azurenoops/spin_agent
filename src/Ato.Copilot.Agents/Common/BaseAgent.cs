            ...
            case AiProvider.Foundry:
                ...
                catch (Exception ex)
                {
                    Logger.LogWarning(ex,
                        "Foundry processing threw exception for agent {AgentName} (configured model: {ConfiguredModel}) — falling back to IChatClient. " +
                        "The response will be generated by the IChatClient deployment instead. " +
                        "If this is unexpected, verify the Foundry agent configuration. (#698)",
                        AgentName, _azureAiOptions.DeploymentName);
                }

                Logger.LogWarning(
                    "Foundry processing returned null for agent {AgentName} (configured model: {ConfiguredModel}) — falling back to IChatClient. " +
                    "Model swap: Foundry → IChatClient. Check Foundry provisioning if this is not intended. (#698)",
                    AgentName, _azureAiOptions.DeploymentName);
