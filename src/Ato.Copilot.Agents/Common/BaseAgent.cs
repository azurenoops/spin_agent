...
public abstract class BaseAgent
{
        ...
        switch (_azureAiOptions.Provider)
        {
            case AiProvider.Foundry:
                ...
                try
                {
                    ...
                    fallbackReason = "Foundry returned no response";
                }
                ...
                if (!_azureAiOptions.AllowBackendFallback)
                {
                    Logger.LogError(
                        "Foundry processing failed for agent {AgentName}; backend fallback is disabled. ConfiguredProvider={ConfiguredProvider}, ConfiguredModel={ConfiguredModel}, FallbackReason={FallbackReason}",
                        AgentName, _azureAiOptions.Provider, _azureAiOptions.DeploymentName, fallbackReason);
                    return null;
                }

                var fallbackResponse = await TryProcessWithAiAsync(
                    message, context, cancellationToken, progress,
                    AiProvider.OpenAi, _azureAiOptions.DeploymentName);
                if (fallbackResponse is null)
                {
                    Logger.LogError(
                        "AI backend fallback failed for agent {AgentName}. ConfiguredProvider={ConfiguredProvider}, ConfiguredModel={ConfiguredModel}, ResolvedProvider={ResolvedProvider}, ResolvedModel={ResolvedModel}, FallbackReason={FallbackReason}",
                        AgentName, _azureAiOptions.Provider, _azureAiOptions.DeploymentName,
                        AiProvider.OpenAi, _azureAiOptions.DeploymentName, fallbackReason);
                    return null;
                }

                Logger.LogError(
                    "AI backend fallback completed for agent {AgentName}. ConfiguredProvider={ConfiguredProvider}, ConfiguredModel={ConfiguredModel}, ResolvedProvider={ResolvedProvider}, ResolvedModel={ResolvedModel}, FallbackReason={FallbackReason}",
                    AgentName, _azureAiOptions.Provider, _azureAiOptions.DeploymentName,
                    AiProvider.OpenAi, _azureAiOptions.DeploymentName, fallbackReason);

                fallbackResponse.BackendProvider = AiProvider.OpenAi.ToString();
                fallbackResponse.BackendModel = _azureAiOptions.DeploymentName;
                fallbackResponse.Warnings.Add(new AgentWarning
                {
                    Code = "AI_BACKEND_FALLBACK",
                    Message = "Azure AI Foundry was unavailable; this response was generated by the approved Azure OpenAI fallback backend."
                });
    ...
    protected async Task<AgentResponse?> TryProcessWithAiAsync(
        ...
        CancellationToken cancellationToken = default,
        IProgress<string>? progress = null,
        AiProvider? resolvedProvider = null,
        string? resolvedModel = null)
    {
        ...
        try
        {
            ...
            for (var round = 0; round < maxRounds; round++)
            {
                ...
                {
                    CallIndex = round,
                    Provider = (resolvedProvider ?? _azureAiOptions.Provider).ToString(),
                    ModelId = resolvedModel ?? _azureAiOptions.DeploymentName,
...
public class AgentResponse
{
    ...
    /// <summary>AI backend that generated this response when backend resolution occurred.</summary>
    public string? BackendProvider { get; set; }

    /// <summary>Model deployment that generated this response when backend resolution occurred.</summary>
    public string? BackendModel { get; set; }

    /// <summary>Structured non-fatal warnings that callers must surface or audit.</summary>
...
/// <summary>Structured warning emitted while processing an agent request.</summary>
